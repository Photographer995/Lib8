...
public class StudentGroupController {
    ...
    public ResponseEntity<String> createGroup(@RequestBody StudentGroup group) {
        return ResponseEntity.badRequest()
                .body("Создание групп вручную запрещено. Группы загружаются из API БГУИР.");
    }

    @PutMapping("/{id}")
    public ResponseEntity<String> updateGroup(@PathVariable Long id, @RequestBody StudentGroup group) {
        return ResponseEntity.badRequest()
                .body("Обновление групп запрещено. Группы загружаются из API БГУИР.");
    }

    @DeleteMapping("/{id}")
    public ResponseEntity<String> deleteGroup(@PathVariable Long id) {
        return ResponseEntity.badRequest()
                .body("Удаление групп запрещено. Группы загружаются из API БГУИР.");
    }
